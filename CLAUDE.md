...
| 문서 | 내용 |
|------|------|
| [`docs/LESSONS_LEARNED.md`](docs/LESSONS_LEARNED.md) | TypeORM `@JoinColumn` 누락, `envFilePath` 설정, pgvector 이미지, 마이그레이션 `.env` 로드 등 실제 발생한 시행착오 규칙 |
| [`docs/PARALLEL_DEVELOPMENT.md`](docs/PARALLEL_DEVELOPMENT.md) | git worktree 기반 병렬 개발 가이드 — worktree 생성/삭제, 모듈 격리 규칙, PR 단위, 머지 순서, 충돌 대응 |
| [`docs/PROJECT_GUIDE.md`](docs/PROJECT_GUIDE.md) | 세션별(A/B/C) 전체 구현 프롬프트 모음 — Step 0 초기 셋업부터 Step 2 통합까지 5주 로드맵 |
| [`apps/api-journal/CLAUDE.md`](apps/api-journal/CLAUDE.md) | api-journal 모듈별 엔드포인트, 인증 방식, LLM 의존성, 테스트 현황 |
| [`apps/api-report/CLAUDE.md`](apps/api-report/CLAUDE.md) | api-report 리포트 생성 흐름, 외부 API 의존성(DART/Naver/Anthropic), 캐시 TTL |
| [`apps/api-backtest/CLAUDE.md`](apps/api-backtest/CLAUDE.md) | api-backtest 구현/미구현 엔드포인트 현황, DSL 구조, T-26 구현 계획 |
| [`apps/web/CLAUDE.md`](apps/web/CLAUDE.md) | 페이지 목록, 프록시 구조, 인증 흐름, 환경변수 |
| [`apps/api-gateway/CLAUDE.md`](apps/api-gateway/CLAUDE.md) | 게이트웨이 현재 스켈레톤 상태 및 향후 통합 계획 |
...
- 모든 api 구현시 service에 비즈니스 로직을 구현해야 하며 컨트롤러는 서비스 로직의 분기를 하고 답변을 리턴한다.
